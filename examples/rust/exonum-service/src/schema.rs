...
encoding_struct! {
        ...
        struct Account {
            /// Public key of the employee owner.
            pub_key: &PublicKey,
            ...
            /// ID document number of the employee.
...
impl Account {
    /// Returns a copy of this account with the changed info.
    ...
    pub fn set_custom_data(self, custom_data: &str) -> Self {
        Self::new(self.pub_key(), self.first_name(), self.last_name(), self.id_number(),
                  custom_data)
    }
}

/// Schema of the key-value storage used by the demo employees service.
pub struct EmployeesSchema<T> {
    view: T,
}
...
impl<T: AsRef<Snapshot>> EmployeesSchema<T> {
    ...
    /// Returns an immutable version of employee ID to account PK table.
